...
Object.entries(theme.semanticTokenColors || {}).forEach(([name, settings]: [string, any]) => {
...
console.log(`   Соответствие: ${issues.length === 0 ? '✅ 100%' : `⚠️  ${Math.round((1 - issues.length / theme.tokenColors.length) * 100)}%`}`)

...
import { generateContrastReport } from './wcag-contrast-checker'

// Run validation on the light theme
function validateLightTheme() {
  console.log(
    'Validating Tokyo Night Modern Light Theme for WCAG compliance...\n'
  )
  ...
  if (nonCompliant.length > 0) {
    ...
    nonCompliant.forEach((item) => {
      console.log(
        `  - ${item.name}: ${item.message} (${item.foreground} on ${item.background})`
      )
    })
  } else {
